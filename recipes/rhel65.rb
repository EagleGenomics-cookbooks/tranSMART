...
# Needed to change for RServe
cookbook_file 'sudoers.patch' do
  path "#{Chef::Config[:file_cache_path]}/sudoers.patch"
  action :create
end
...
bash 'Install_tranSMART' do
  ...
  code <<-EOH
    ...
    patch -b -N --verbose /etc/sudoers #{Chef::Config[:file_cache_path]}/sudoers.patch
    ...
    echo 'USER=tomcat7' | sudo tee /etc/default/rserve # not sure this is needed in this version?
    ...
    patch -b -N --verbose /usr/share/apache-tomcat-7/tomcat/.grails/transmartConfig/Config.groovy #{Chef::Config[:file_cache_path]}/configGroovy.patch
